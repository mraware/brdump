...
class Dump {
  ...
  dumpAll = async () => {
    try {
      ...
      const propsRegExp = /^(?<index>\d+)\) (?<prop>.*)$/;
      ...
      if (classesMatch.length) {
        for (let i = 0; i < classesMatch.length; i++) {
          ...
          const propsMatch = await this.omegga.watchLogChunk(`ListProps ${className} *`, propsRegExp, {first: 'index'});
          if (propsMatch) {
            const data = propsMatch.reduce((prev, curr) => prev+curr.groups.prop+'\n', `${className}\n\n`);
        ...
        files.forEach(({data, folderPath, type, className}, index) => {
          console.log(`Write: ${index+1}/${classesMatch.length}: ${className}`);
